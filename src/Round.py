...
#        surface.convert_alpha()
    ...
    def mainloop(self):
        global punch_count, kick_count, forward_count, backward_count, punch, kick, forward, backward
        ...
        while True:
            ...
            upper_bound = np.array([255, 255, 212])
            mask = cv2.inRange(hsv, lower_bound, upper_bound)

            image = cv2.line(image, (100, 0), (100, 480), (0, 255, 0), 2)
            image = cv2.line(image, (300, 0), (300, 480), (0, 255, 0), 2)
            ...
            for i in range(50, 476):
                for j in range(290, 300):
                    if (mask[i][j]) == 255:
                        forward_count += 1

            if forward_count >= 3500:
                forward = 1


            for i in range(350, 476):
                for j in range(80, 100):
                    if (mask[i][j]) == 255:
                        backward_count += 1

            if backward_count >= 2000:
                backward = 1
...
class InputReader:
    ...
    def getInputs(self):
        global punch, kick, forward, backward
        btn_inputs_p1 = []
        stick_inputs_p1 = []
        btn_inputs_p2 = []
        stick_inputs_p2 = []
